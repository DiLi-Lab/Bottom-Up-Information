...
class Transformer(nn.Module):
    ...
    def __init__(self, args):
        ...
        # self.part_label_embed = nn.Embedding(3, 1024)

        # self.film_gamma = nn.Linear(1024, 1024)
        # self.film_beta = nn.Linear(1024, 1024)
        if args.part_labeled_img:
            if args.revealed_part == 'random':  
                self.part_label_embed = nn.Embedding(3, 1024)
                self.film_gamma = nn.Linear(1024, 1024)
                self.film_beta = nn.Linear(1024, 1024)
            else: # e.g., only "f" is used
                self.part_label_embed = nn.Embedding(3, 1024)
                nn.init.zeros_(self.part_label_embed.weight)
                self.film_gamma = nn.Linear(1024, 1024)
                self.film_beta = nn.Linear(1024, 1024)
                with torch.no_grad():
                    self.film_gamma.weight.zero_()
                    self.film_gamma.bias.fill_(1.0)  # so gamma = 1
                    self.film_beta.weight.zero_()
                    self.film_beta.bias.zero_()      # so beta = 0
    ...
    def forward(self, image, text_length, text_input, part_label=None, radical_length=None, radical_input=None, conv_feature=None, test=False, att_map=None):

        if conv_feature is None:
            conv_feature = self.encoder(image)
        # if part_label is not None:
        #     label_feat = self.part_label_embed(part_label)  # shape (B, 1024)
        #     b, c, h, w = conv_feature.shape
        #     label_feat = label_feat.view(b, c, 1, 1).expand(-1, -1, h, w)
        #     conv_feature = conv_feature + label_feat
        if part_label is not None:
            label_feat = self.part_label_embed(part_label)  # shape (B, 1024)
            gamma = self.film_gamma(label_feat)  # shape: (B, 1024)
            beta = self.film_beta(label_feat)    # shape: (B, 1024)
            b, c, h, w = conv_feature.shape
            gamma = gamma.view(b, c, 1, 1).expand(-1, -1, h, w)
            beta = beta.view(b, c, 1, 1).expand(-1, -1, h, w)
            conv_feature = gamma * conv_feature + beta
