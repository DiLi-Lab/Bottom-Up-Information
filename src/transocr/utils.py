import torch
from data.dataset import lmdbDataset, resizeNormalize, motrDataset, lmdbDataset_singleCharExtracted, trdgDataset
...
def get_dataloader(root, args, shuffle=False):
    if root.endswith('pkl'):
        ...
        dataset = trdgDataset(root, resizeNormalize((args.imageW, args.imageH)))
        dataloader = torch.utils.data.DataLoader(
            dataset, batch_size=args.batch_size, shuffle=shuffle, num_workers=8,
        )
    elif root.endswith('tsv'):
        # dataset = motrDataset(root, resizeNormalize((args.imageW, args.imageH)), args.revealed_part)
        dataset = motrDataset(root=root, 
                              transform=resizeNormalize((args.imageW, args.imageH)), 
                              revealed_part=args.revealed_part, 
                              extract_single_char=args.extract_single_char)
        dataloader = torch.utils.data.DataLoader(
            dataset, batch_size=args.batch_size, shuffle=shuffle, num_workers=8,
        )
    else:
        if args.extract_single_char:
            dataset = lmdbDataset_singleCharExtracted(root,resizeNormalize((args.imageW, args.imageH)), args.img_from_lbl, args.revealed_part, args.part_labeled_img)
        else:
            dataset = lmdbDataset(root,resizeNormalize((args.imageW, args.imageH)), args.img_from_lbl, args.revealed_part, args.part_labeled_img)
        dataloader = torch.utils.data.DataLoader(
            dataset, batch_size=args.batch_size, shuffle=shuffle, num_workers=8,
        )
